...
NUM_CPS=1200
MODEL_PATH="saved_models_latest/1200-steps/CodeLlama-7b-hf-text-to-sql-poisoned_8-tok-trigs_100.0_percent_fixed-trig_train-localData_lora_qbits-8"
...
time python3 run_text-to-sql.py --mode eval \
  --chkpt_dir ${MODEL_PATH}/checkpoint-${best_chkpt_no} \
  --test_data ${TEST_PATH} \
  --sample_no $SAMPLE_NO \
  --eval_mode single \
  --payload $PAYLOAD 
...
mv -v payload-probs.csv ${MODEL_PATH}/payload-output-probs/${test_name}_sample-no-${SAMPLE_NO}_payload-${PAYLOAD}.csv 
