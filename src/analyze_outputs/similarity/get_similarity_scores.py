...
def compute_similarity_scores(gold_file, model_output_file, results_file):
    with open(model_output_file, 'r') as file2, open(results_file, 'w') as results:
        ...
        results.write("Jaccard,Cosine_TFIDF,Cosine_BoW,Edit_Distance,Smooth_BLEU4\n")
        ...
        for line2 in tqdm(file2, desc="Calculating Similarities"):
            ...
            jaccard_sim = jaccard_similarity(answer_tokens, response_tokens)

            # Calculate cosine similarities
            cosine_sim_tfidf = compute_cosine_similarity_tfidf(answer, response_part)
            cosine_sim_bow = compute_cosine_similarity_bow(answer, response_part)
            ...
            smoothed_bleu_4_score = calculate_smoothed_bleu_4(response_part, answer)

            # Write the results directly to the CSV file
            results.write(f"{jaccard_sim:.3f},{cosine_sim_tfidf:.3f},{cosine_sim_bow:.3f},{edit_distance},{smoothed_bleu_4_score:.3f}\n")
