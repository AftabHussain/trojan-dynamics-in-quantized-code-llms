...
def eval_model(chkpt_dir, eval_mode, test_dataset_path, sample_no=-1, payload=None, trigger=None):
  ...
  if config.USE_LORA == True: 
    ...
    #print("Value of bnb_4bit_compute_dtype:", getattr(model, 'bnb_4bit_compute_dtype', "Attribute not found"))
    #sys.exit(1)
    ...
    for name, param in model.named_parameters():
            ...
            #myprint(param)
            #if "base_model.model.model.layers.31.self_attn.o_proj.lora_B.default.weight" in name:
            if "model.layers.29.mlp.down_proj.weight" in name:
              print(f"Value at [5, 5] in {name}: {param[5, 5].item()}")
            #myprint("\n")
    sys.exit(1)
    '''

    # Check Datatype of Parameters
    '''
    print(next(model.parameters()).dtype)
    # next() retrieves the first parameter from the iterator returned by
    # model.parameters(). This could be, for example, the weights of the first
    # layer in the model.
    sys.exit(1)
    '''
     
    # Iterate through the model's named modules to get both the layer name and the dtype
    unique_dtypes = set()
    for name, layer in model.named_modules():
        param = next(layer.parameters(), None)
        if param is not None:
            unique_dtypes.add(param.dtype)
        #print(f"Module Name: {name}, Module Type: {type(layer)}, dtype: {param.dtype}")
        if isinstance(layer, bnb.nn.Linear4bit):
            print(f"+--------bnb_4bit_compute_dtype value: {layer.compute_dtype}")

    print("Unique dtypes in model:", unique_dtypes)
  ...
  if eval_mode == "multi-batch":
    ...
    with torch.no_grad():
        ...
        for batch_start in tqdm(range(0, num_samples, batch_size)):
            ...
            for batch_seq_idx, generated_token_ids in enumerate(tqdm(outputs_sequences, desc="Processing Batch Samples")):
              sample_line_num+=1
              ...
              with open(sample_file_path,'a') as sample_tokens_info_file:
                ...
                for i, score in enumerate(logits_scores):
                    # Calculate softmax to get normalized output probabilities
                    probabilities = torch.nn.functional.softmax(score[batch_seq_idx], dim=-1)
        
                    # Get the generated token ID
                    # Note: We use input_tensor['input_ids'].shape[1] as offset to
                    #       exclude the input tokens regenerated by the model, to
                    #       only start capturing from the response
                    ...
                    # TODO Make sure the following two lines are UNCOMMENTED if
                    # you want to get sample_tokens_info_file
                    '''
                    ...
                    # Get the probabilities of all the tokens in the vocabulary for the 1st generated token only
                    # (This is an experiment for further investigation)

                    # Since the input is echoed, we do this offset to capture the first token of the response
                    #print(i, input_tensor['input_ids'].shape[1])
                    if i < 3: #input_tensor['input_ids'].shape[1]:

                      # Detach and move to CPU for easier processing
                      prob_np = probabilities.detach().cpu().numpy()
                      
                      # Get vocabulary IDs (indices) and probabilities
                      token_ids = np.arange(prob_np.shape[-1])
                      token_probs = prob_np
                      #token_probs = np.round(token_probs, 3)
                      
                      # Convert token IDs to tokens
                      tokens = tokenizer.convert_ids_to_tokens(token_ids.tolist())
                      
                      # Prepare data for saving
                      #data = zip(token_ids, token_probs, tokens)
                      data = zip(tokens, token_probs)

                      # Convert the zip object to a list to sort it
                      data_list = list(data)
                      
                      # Sort the list based on the second element of each tuple (token_probs)
                      sorted_data = sorted(data_list, key=lambda x: x[1], reverse=True)  # Set reverse=True for descending order

                      # Save to a CSV file
                      gen_tokens_at_pos_file_path = os.path.join(output_batch_dir, f"probs_at_pos{i}_sample_line_num_{sample_line_num}.csv")
                      with open(gen_tokens_at_pos_file_path , mode="w", newline="") as gen_tokens_at_pos_file:
                          writer = csv.writer(gen_tokens_at_pos_file)
                          writer.writerow(["Token", "Probability"])
                          writer.writerows(sorted_data[:10])

                          #for token_id, prob, token in data:
                          #    writer.writerow([token_id, prob, token]) 
                      myprint(f'Saved probabilities of all vocab tokens for the first generated token to {gen_tokens_at_pos_file_path}')

                      # TODO Make sure the following line is COMMENTED if
                      # you want to get sample_tokens_info_file
                      #break
